...
void IRCommunicatie::encodingToTime(int *Byte){
	for (int i=0; i<LengthArrayBits; i++)												//loops through all LENGTHARRAYBITS of data array
	{
		if(Byte[i] == 1){													//if the bit is 1
			dummyTimes[i] = Devines02OverFlowCounterBit1;							//turn the 1 into required define time for bit1 and put the value in another array
			//Serial.println(dummyTimes[i]);									//debug
			} else if(Byte[i] == 0){										//as above, but than for the bit value 0
			dummyTimes[i] = Devines02OverFlowCounter0;
			//Serial.println(dummyTimes[i]);									//debug
...
void IRCommunicatie::encodetimeToLED(int *Times){
	if(startBitActive && !stopBitActive && !parityBitActive){				//Check for sending start bit or not
		...
		if(getCounterSENDING() == Devines02OverFlowCounterStartBit){
			nextBit = true;
		...
		nextBitQuestionMark();
		if(getCounterSENDING() == Devines02OverFlowCounterStopBit){
			nextBit = true;
		...
		nextBitQuestionMark();
		if(getCounterSENDING() == Devines02OverFlowParityBit){
			nextBit = true;
