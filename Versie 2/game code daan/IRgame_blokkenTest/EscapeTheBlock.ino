...
int invinccheck;		// check die ervoor zorgt dat de invincibility maar 1 keer word geregeld, ipv idere loep weer
...
void start(){
	...
	joyx2 = 125 - joyx + 125;
	//drawcharacter2(joyx2, 300, speler2kleur);

	
	drawblock(locaties[nummer1], groottes[nummer1]);		//teken 2 blokken
	drawblock2(locaties[nummer2], groottes[nummer2]);
	
	collision();		//check of er een botsing is
	header();			//teken de header
	
	if (MAXLEVENS - geraakt <= 0) {		//als de levens op zijn, ga naar het gameover scherm
		scherm = 5;
	}
...
void drawblock2(int x, int grootte) {
	...
	if (score >= 125) {
		...
		spelersnelheid = 7;			// snelheid van de speler ( 7 = snel, 3 = sloom)
	} else if (score >= 75) {
	...
	if (score >= 25) {
		snelheid = 4;				// de blokken vallen sloom
		blokweg = 19;				// de grootte van het zwarte blok wat de blokke achtervolgt om het spoor weg te halen
...
void collision (){
	if (checkcollision == 0){
		...
		/*
		// charachter 2
		if(locaties[nummer1] + groottes[nummer1] >= charachterx2 - 15 && locaties[nummer1] <= charachterx2 - 15 && (stap1 >= 265 && stap1 <= 285)) {
			checkcollision = 1;
			collisiontijd = score;
			geraakt++;
		}
		...
		if(locaties[nummer2] + groottes[nummer2] >= charachterx2 - 15 && locaties[nummer2] <= charachterx2 - 15 && (stap2 >= 265 && stap2 <= 285)) {
			collisiontijd = score;
			geraakt++;
		}
	...
	if (checkcollision == 1 && invinccheck == 0) {	//er is een botsing geweest
		blokweg = blokweg + 3;
		screen.fillRect(0, 0, 160, 10, BACKGROUND);
		speler1kleur = WHITE;
		invinccheck = 1;
	}
	if (checkcollision == 1 && (collisiontijd + invincibility) < score) {
		checkcollision = 0;
		speler1kleur = BLUE;
	}
	/*		UITLEZEN
...
void drawcharacter2(int x, int y, int Color){
	...
	if (x > 140 && charachterx2 < 225) {			//de x coordinaten van de charachter mag niet hoger worden dan 225, anders gaat hij uit het scherm
		charachterx2 = charachterx2 + 4;								//als de nunchuck naar rechts is geduwd, gaat het charachter ook naar rechts
		...
		charachterx2 = charachterx2 - 4;								//als de nunchuck naar links is geduwd, gaat het charachter ook naar links
	}
